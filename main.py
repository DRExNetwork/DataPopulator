...
def main():
    ...
    data_project_environmental = load_json_data(
        "project_environmental_information.json"
    )
    ...
    data_project_developer_assginment = load_json_data(
        "project_developer_assginment.json"
    )
    ...
    print("Enter the number of entries you want to insert (max 5)")
    num = int(input())
    if num > 5:
        print("Number of entries cannot be greater than 5. Exiting script.")
        exit()

    uid = create_auth_users(None)
    print("this is user id", uid)
    create_auth_identities(None)
    locations = [
        (d["country"], d["city"], d["district"], d["postal_code"])
        for d in data_locations
        for _ in range(num)
    ]
    location_id = create_locations(locations, True)
    ...
    financiers = [
        (
            ...
            d["company_name"],
            d["tax_id"],
            d["representative_role"],
            d["webpage"],
            d["google_maps_location"],
            d["telephone"],
            d["profile_picture"],
        )
    ...
    beneficiaries = [
        (
            ...
            financier_id,
            d["beneficiary_name"],
            d["beneficiary_address_1"],
            d["beneficiary_address_2"],
        )
    ...
    financier_receipt_info = [
        (financier_id, d["name"], d["tax_id"], d["address"], d["email"], d["phone"])
    ...
    projects = [
        (
            ...
            d["description"],
            d["nominal_capacity"],
            d["estimated_annual_energy"],
            d["stage"],
            d["type"],
            d["technology"],
            d["power_plant_amount"],
            d["days_to_participate"],
            d["num_sponsors"],
            d["num_financiers"],
            d["ppa_duration"],
            d["admin_listing_date"],
            d["financing_stage"],
        )
        for d in data_projects
        for i in range(num)  # This will duplicate each tuple
    ]
    print(projects)

    project_id = create_projects(projects, True)
    ...
    projects_development = [
        (
            ...
            d["panel_count"],
            d["panel_brand"],
            d["energy_cost_annual"],
            d["energy_maintenance_cost_annual"],
            d["energy_tariff"],
            d["projected_income"],
            d["maintenance_cost_annual"],
            d["insurance_cost_annual"],
            d["drex_cost_annual"],
            d["radiation_onsite"],
            d["technical_memo"],
            d["technical_blueprint"],
            d["technical_equipment_specs"],
            d["operation_estimate"],
            d["construction_starts_estimate"],
            d["legal_document"],
            d["commercial_offer"],
            d["intention_letter"],
            d["insurance_funding"],
            d["insurance_completion"],
            d["insurance_all_risks"],
            d["insurance_energy_generation"],
    ...
    project_development_id = create_project_development_information(
        projects_development, True
    )
    print("Project Development ID:", project_development_id)

    project_env = [
        (project_id[i], d["trees_planted_estimated"], d["co2_removed_stimated"])
        for d in data_project_environmental
        for i in range(num)
    ]

    project_env_id = create_project_environmental_information(project_env, True)
    ...
    project_finance = [
        (
            ...
            d["aggregate_raised"],
            d["debt_duration"],
            d["estimated_annual_return"],
        )
        for d in data_project_financial
        for i in range(num)
    ]

    project_finance_id = create_project_financial_information(project_finance, True)
    ...
    project_maintenance_id = create_project_maintenance_information(
    ...
    investor_project_assign_id = create_investor_project_assignments(
        investor_project_assign, True
    )
    print("Investor Project Assign ID:", investor_project_assign_id)

    financier_payments = [
        (investor_project_assign_id[i], d["amount"], d["order"], d["payment_date"])
        for d in data_financier_payment
        for i in range(num)
    ]

    financier_payment_id = create_financier_payments(financier_payments, True)
    print("Financier Payments ID:", financier_payment_id)

    financier_receipt_details = [
        (
            investor_project_assign_id[i],
            d["transfer_id"],
            d["transferred_payment_timestamp"],
            d["registered_payment_timestamp"],
            d["expected_payment_timestamp"],
            d["amount"],
            d["state"],
            d["pdf"],
            d["bank_fee"],
    ...
    financier_receipt_details_id = create_financier_receipt_details(
        financier_receipt_details, True
    )
    print("Financier Receipt Details ID:", financier_receipt_details_id)

    project_developer_assignments = [
        (project_id[i], developer_id, d["assigned_date_time"], d["disbursement_count"])
        for d in data_project_developer_assginment
        for i in range(num)
    ]

    project_developer_assignments_id = create_project_developer_assignments(
        project_developer_assignments, True
    )
    print("Project Developer Assignments ID:", project_developer_assignments_id)

    disbursements = [
        (
            project_developer_assignments_id[i],
            d["amount"],
            d["status"],
            d["description"],
            d["date"],
    ...
    disbursements_id = create_disbursement(disbursements, True)
    ...
    sme_project_assignment_id = create_sme_project_assignment(
        sme_project_assignments[0]
    )
    print("SME Project Assignment ID:", sme_project_assignment_id)

    sme_deposits = [
        (
            sme_project_assignment_id,
            d["amount"],
            d["remaining"],
            d["status"],
            d["date"],
    ...
    sme_deposit_id = create_sme_deposit(sme_deposits, True)
    print("SME Deposits ID:", sme_deposit_id)

    sme_loan_repayments = [
        (sme_project_assignment_id, d["progress"], d["sme_deposit_energy"])
        for d in data_sme_loan_repayments
        for _ in range(num)
    ]

    sme_loan_repayment_id = create_sme_loan_repayment(sme_loan_repayments, True)
